...
export default function Discover() {
  ...
  if (!items.length) return <div className="flex items-center justify-center min-h-screen text-gray-500">No more cards!</div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="p-6">
            <h2 className="text-2xl font-bold mb-2">{item.user?.firstName} {item.user?.lastName} || {item.companyName}</h2>
            <p className="text-gray-600 mb-4">{item.user?.locationCity}, {item.user?.locationState}</p>
